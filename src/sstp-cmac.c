...
void sstp_cmac_result(cmac_ctx_st *ctx, uint8_t *msg, int mlen, uint8_t *result, int length)
{
    ...
    unsigned int klen32 = (unsigned int)sizeof(key);
    unsigned int outlen32 = 0;
    uint8_t iter = 0x01;
    ...
    /* Use EVP_MAC (OpenSSL 3.x) to perform HMAC in a provider-friendly way */
    {
        const char *mdname = (SSTP_CMAC_SHA256 & ctx->flag) ? "SHA256" : "SHA1";
        ...
        if (!EVP_MAC_init(mctx, ctx->key, sizeof(ctx->key), params))
        {
            EVP_MAC_free(mac);
            EVP_MAC_CTX_free(mctx);
            return;
        }
        if (!EVP_MAC_update(mctx, ctx->seed, ctx->slen) ||
            !EVP_MAC_update(mctx, (const unsigned char *)&len, sizeof(len)) ||
            !EVP_MAC_update(mctx, (const unsigned char *)&iter, sizeof(iter)))
        {
            EVP_MAC_free(mac);
            EVP_MAC_CTX_free(mctx);
            return;
        }
        if (!EVP_MAC_final(mctx, key, &klen, sizeof(key)))
        {
            EVP_MAC_free(mac);
            EVP_MAC_CTX_free(mctx);
            return;
    ...
    outlen32 = (unsigned int)length;
    HMAC_Final(&hmac, result, &outlen32);
    HMAC_CTX_cleanup(&hmac);
#else
#if OPENSSL_VERSION_NUMBER >= 0x30000000L
    /* EVP_MAC path (OpenSSL 3.x) */
    {
        const char *mdname = (SSTP_CMAC_SHA256 & ctx->flag) ? "SHA256" : "SHA1";
        EVP_MAC *mac = EVP_MAC_fetch(NULL, "HMAC", NULL);
        EVP_MAC_CTX *mctx = NULL;
        ...
        params[0] = OSSL_PARAM_construct_utf8_string("digest", (char *)mdname, 0);
        params[1] = OSSL_PARAM_construct_end();
        if (!EVP_MAC_init(mctx, key, klen, params))
        {
            EVP_MAC_free(mac);
            EVP_MAC_CTX_free(mctx);
            return;
        }
        if (!EVP_MAC_update(mctx, msg, mlen))
        {
            EVP_MAC_free(mac);
            EVP_MAC_CTX_free(mctx);
            return;
        }
        if (!EVP_MAC_final(mctx, result, &outlen, (size_t)length))
        {
            EVP_MAC_free(mac);
            EVP_MAC_CTX_free(mctx);
            return;
